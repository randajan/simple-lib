(() => {
  ...
  jet.type = new Complex_default((any) => _id(null, any), {
    ...
    define: (name, constructor, opt, custom9) => {
      ...
      return jet[name] = new Complex_default(create, fix, custom9);
    }
  });
  var jet_default = jet;
  var custom = {
    prop: {
      add: function(obj, property, val, writable, enumerable, overwrite) {
        overwrite = jet_default.Bol.tap(overwrite, true);
        if (jet_default.type.isMapable(property)) {
          jet_default.Map.it(property, (f, i) => {
            const n = jet_default.Str.isNumeric(i);
            jet_default.Obj.prop.add(obj, n ? f : i, n ? val : f, writable, enumerable, overwrite);
          });
      ...
      get: function(obj, property) {
        ...
        const props = {};
        jet_default.Map.it(property, (k) => props[k] = obj[k]);
        return props;
      }
    },
    json: {
      from: function(json, throwErr) {
        if (jet_default.type.isMapable(json)) {
          return json;
        }
        try {
          return JSON.parse(jet_default.Str.to(json));
        } catch (e) {
          if (throwErr === true) {
            throw e;
          }
      ...
      to: function(obj, prettyPrint) {
        const spacing = jet_default.Num.only(prettyPrint === true ? 2 : prettyPrint);
        return JSON.stringify(jet_default.type.isMapable(obj) ? obj : {}, null, spacing);
      }
    }
  };
  var Obj_default = jet_default.type.define("Obj", Object, {
    rank: -6,
    is: (x, t) => t === "object",
  ...
  jet_default.Obj.to.define({
    Fce: (obj) => (_) => obj,
    Symbol: (obj) => Symbol(jet_default.Obj.json.to(obj)),
